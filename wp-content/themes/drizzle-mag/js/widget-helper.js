var drizzle_mag_upload_frame_obj;
...
( function( $ ) {
	...
	jQuery(document).ready(function($) {
		...
		jQuery(document).on('click', 'input.wh-image-picker', function( event ){
			...
			var DrizzleMagImageObject = wp.media.controller.Library.extend({
				defaults : _.defaults({
					id: 'drizzle-mag-state-insert',
			...
			drizzle_mag_upload_frame_obj = wp.media.frames.drizzle_mag_upload_frame_obj = wp.media({
				button: {
					text: jQuery( this ).data( 'uploader_button_text' )
				},
				state : 'drizzle-mag-state-insert',
				states : [
					new DrizzleMagImageObject()
				],
				multiple: false
			});

			drizzle_mag_upload_frame_obj.on( 'select', function() {

				var state = drizzle_mag_upload_frame_obj.state('drizzle-mag-state-insert');
			...
			drizzle_mag_upload_frame_obj.open();
		});
